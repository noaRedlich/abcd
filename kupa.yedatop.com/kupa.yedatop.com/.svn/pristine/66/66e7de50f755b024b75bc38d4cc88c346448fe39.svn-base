<?php
	...
	function getMainListingData($listingID)
	{
		...
		global $conn, $lang;
		$listingID = make_db_extra_safe($listingID);
		$sql = "SELECT listingsDB.user_ID, listingsDB.Title, listingsDB.expiration, UserDB.user_name FROM listingsDB, UserDB WHERE ((listingsDB.ID = $listingID) AND (UserDB.ID = listingsDB.user_ID))";
		$recordSet = $conn->Execute($sql);
		if ($recordSet === false)
		{
			log_error($sql);
		...
		echo "<h4>$lang[listed_by] ".show_profile($listing_user_ID)."</h4>";
	...
	function searchbox_pulldown ($browse_caption, $browse_field_name)
	{
	...
	function latestListings($num_of_listings)
	{
		...
		global $conn;
		echo "<ul>";
		$sql = "SELECT ID, Title FROM listingsDB WHERE (listingsDB.active = 'yes') ORDER BY creation_date DESC";
		$recordSet = $conn->SelectLimit($sql, $num_of_listings, 0);
		if ($recordSet === false)
		{
			log_error($sql);
