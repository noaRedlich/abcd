<?php
	...
	<?php
			...
			<?php
				...
				$sql="SELECT name as user_name FROM users as UserDB WHERE (id = '$edit_owner')";
				$ADODB_FETCH_MODE = ADODB_FETCH_ASSOC;
				$recordSet = $conn->Execute($sql);
				if ($recordSet === false)
				{
				...
				$sql="SELECT id as ID, name as user_name FROM users as UserDB ORDER BY user_name";
				$ADODB_FETCH_MODE = ADODB_FETCH_ASSOC;
				$recordSet = $conn->Execute($sql);
				if ($recordSet === false)
				{
	...
	//	$sql = "SELECT listingsDBElements.field_name, listingsDBElements.field_value, listingsFormElements.field_type, listingsFormElements.field_caption, listingsFormElements.default_text, listingsFormElements.field_elements, listingsFormElements.required FROM listingsDBElements, listingsFormElements WHERE ((listingsDBElements.listing_id = '$edit') AND (listingsDBElements.field_name = listingsFormElements.field_name)) ORDER BY rank";
		$sql ="SELECT f.field_name, db.field_value, f.field_type, f.field_caption, f.default_text, f.field_elements, f.required FROM listingsFormElements f left join listingsDBElements db on db.field_name = f.field_name and db.listing_id = '$edit' ORDER BY rank";
	...
	if ($edit == "")
	{
		...
		$sql = "SELECT listingsDB.ID, listingsDB.Title, listingsDB.notes, listingsDB.expiration,listingsDB.active, UserDB.emailAddress as email FROM listingsDB, UserDB where listingsDB.user_ID = UserDB.ID ORDER BY listingsDB.ID ASC";
		...
		?>
			<table border="<?php echo $style[admin_listing_border] ?>" cellspacing="<?php echo $style[admin_listing_cellspacing] ?>" cellpadding="<?php echo $style[admin_listing_cellpadding] ?>" width="<?php echo $style[admin_table_width] ?>" class="form_main" style='border-collapse:collapse'>
		<?php
