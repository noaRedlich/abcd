...
class DataList
{
	...
	// get post and get variables
		global $HTTP_SERVER_VARS, $HTTP_GET_VARS, $HTTP_POST_VARS, $_SERVER;
	...
	// retouch query to avoid case matching problems
		...
		$this->HTML .= @implode ("", @file ("./header.html"));
	...
	// display table title and Print and Filter icons
		...
		$this->HTML .= "<table width=" . $sum_width . " cellspacing=0 cellpadding=0 border=0><tr><td align=left><div class=TableTitle>$this->TableTitle</div>";
	...
	if(!$this->HideTableView)
	{
		...
		{
			$this->HTML .= "<div  style='text-align:" . ( ($this->QuickSearch == 1) ? "left" : (($this->QuickSearch == 2) ? "center" : "right")) . ";'>";
		...
		if($this->AllowPrinting && !$Print_x)
			$this->HTML .= "<input type=image src=print.gif name=Print alt='Printer Friendly View'>";
	...
	// end of data
		...
		$this->HTML .= "\n\t<tr><td colspan=$FieldCount><div class=TableFooter>Records $FirstRecord to $i of $RecordCount</div></td></tr>";
	}
