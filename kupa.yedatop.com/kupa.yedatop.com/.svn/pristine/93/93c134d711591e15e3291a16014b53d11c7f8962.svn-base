<?php
	...
	function renderFormElement($field_type, $field_name, $field_caption, $default_text, $field_elements, $required)
		...
		switch ($field_type)
		{
			...
			case "textarea": // handler for textarea boxes
				...
				echo "</b></td>";
				echo "<td  class=\"row_main\"><textarea name=\"$field_name\" cols=40 rows=6>$default_text</textarea></td>";
				break;
			case "select": // handler for select boxes
				...
				$header = "<select name=\"$field_name\" size=\"1\" $event>";
				echo $header;
				
				$scriptstr = "";
				if (!strpos($field_elements,"*")){
				...
				else
				{
					...
					while (list($globalValue, $global_list_item) = each ($global_list))
					{
						...
						while (list($indexValue, $list_item) = each ($index_list))
						{
							...
							$s.="<option value='$list_item'>$list_item";
						}
	...
	function renderExistingFormElement($field_type, $field_name, $field_value, $field_caption, $default_text, $required, $field_elements)
	{
		...
		switch ($field_type)
		{
			...
			case "text": // handles text input boxes
				...
				echo "</b></td>";
				echo "<td  class=\"row_main\"><input type=\"text\" name=\"$field_name\" value=\"$field_value\" width=\"100\"";
				if ($field_type == 'date')
				{
			...
			case "textarea": // handles textarea input
				...
				echo "</b></td>";
				echo "<td  class=\"row_main\"><textarea name=\"$field_name\" cols=40 rows=6>$field_value</textarea></td>";
				break;
			...
			case "price": // handles price input
				...
				echo "</b></td>";
				echo "<td  class=\"row_main\">$money_sign<input type=\"text\" name=\"$field_name\" value=\"$field_value\" width=\"100\"> .00 </td>";
				break;
			case "url": // handles url input fields
	...
	{ // handles multiple page listings
		...
		echo "<table bgcolor=buttonface border=\"0\" cellspacing=\"0\" cellpadding=\"2\" width=\"750\"><TR><TD align=left>";
		if ($num_rows == 1)
		{
