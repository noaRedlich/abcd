...
{		
	...
	var id_card=$('#popupFrame').find('#CardAddID').val();
	if( id_card.split('?').length>1&&id_card.search(';')!=-1){
		id_card= id_card.split(';')[1];
		id_card= id_card.split('?')[0];
   	}
   	if( id_card.split('?').length>1&&id_card.search('ף')!=-1){
		id_card= id_card.split('ף')[1];
		id_card= id_card.split('?')[0];
   	}
    $.post('functions.php',{
    	UseFunc:'CardAdd',
