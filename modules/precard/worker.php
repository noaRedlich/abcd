        ...
        function updateids() {
            ...
            for (i = 0; i < document.F.elements.length; i++) {
                if (document.F.elements[i].timeid + "" != "undefined") {
                    ...
                    document.F.elements[i].parentElement.style.padding = 2;
                    document.F.elements[i].parentElement.innerHTML = document.F.elements[i].parentElement.innerHTML + " " + document.F.elements[i].value;
                    document.F.elements[i].value += "#" + document.F.elements[i].timeid;
                }
            }
            return true;
        }
        ...
        <div id=ADD style='display:none;position:absolute;top:50;left:300;width:200;height:100;background-color:lightyellow;border:outset 2'>
            <table width=100%>
                ...
                <tr>
                    <td>תאריך</td>
                    <td width=1% nowrap>
                        <input size=6 name=newDate id=newDate value="">
            ...
            <tr>
                ...
                <td width=99% style='border:inset 1'>
                    <table id=INFO <?= ($UPDATE) ? "style=display:none" : "" ?> align=left>
                        <tr>
                            <td bgcolor=lightyellow align=center style='padding:0 5;border:solid 1 black;color:blue'><strong>לחץ\י על משבצת
                                    <br>
                                    לעדכון זמן</strong></td>
                        </tr>
                    </table>
...
'>
                    ...
                    <input type=button onclick='<?= "s=window.open(\"recalc.php?num=$num&sDate=$sDate&eDate=$eDate\",\"recalc\",\"top=100,left=100,width=300,height=200\");s.focus();" ?>' value=" חשב שכר "
                           style=";cursor:hand;padding:0 0 0 10;background:url(<?= $imgPath ?>refresh.gif);background-repeat:no-repeat;background-position:left top">

                    <input type=button id=ADDROW onclick='document.getElementById("ADD").style.display=""' value='הוסף שורה'
                           style=';width:100;PADDING-RIGHT: 0px; PADDING-LEFT: 10px;BACKGROUND: url(<?= $imgPath ?>plus.gif) no-repeat left top;PADDING-BOTTOM: 0px; CURSOR: hand; PADDING-TOP: 0px'>
                    <input type=submit id=UPDATE name=UPDATE onclick='return updateids()' value='עדכן'
                           style='display:none;width:100;PADDING-RIGHT: 0px; PADDING-LEFT: 10px;BACKGROUND: url(<?= $imgPath ?>refresh.gif) no-repeat left top; BACKGROUND-color:lightgreen;PADDING-BOTTOM: 0px; CURSOR: hand; PADDING-TOP: 0px'>
...
&nbsp;&nbsp;	ת.ז.: <b>" . $worker->fields["TeudatZehut"] . "</b>";
    ...
    foreach ($report as $row) {
        ...
        <td valign=middle>&nbsp;";
        echo " <img src=" . $imgPath . "delete.gif  align=absmiddle onclick='if (confirm(\"למחוק שורה?\")){document.F.deleterow.value=" . $row["id"] . ";document.F.submit();}' style='cursor:hand' alt='מחק שורה'>";
        echo "</td></tr>";
