<?php
	...
	//handles everything to do with users logging in
	{
		...
		if ($user["credit_user"]){
			$user = $users->get_user($user["credit_user"]);
			$userID = $user["id"];
			$username = $user[username];
		}
	...
	function renderFormElement($field_type, $field_name, $field_caption, $default_text, $field_elements, $required,$visible)
		...
		global $lang,$conn,$userID,$action;
		...
		switch ($field_type)
		{
			...
			case "text": // handler for regular text boxes
				...
				echo "</b></td>";
				echo "<td  class=\"row_main\"><input style='width:230px' type=\"text\" name=\"$field_name\" value=\"".(($action=="create_new_listing")?$_POST[$field_name]:$default_text)."\" width=\"100\"></td>";
				break;
			case "textarea": // handler for textarea boxes
				...
				echo "</b></td>";
				echo "<td  class=\"row_main\"><textarea style='width:230px' name=\"$field_name\" cols=40 rows=6>$default_text</textarea></td>";
				break;
			case "select": // handler for select boxes
				...
				if (strpos(" ".$field_elements,"{")=="1"){
						...
						while (!$recordS->EOF){
							echo "<option value=\"".$recordS->fields["ID"]."\"  ".(($_GET[$field_name]==$recordS->fields["ID"]||$_POST[$field_name]==$recordS->fields["ID"])?"selected":"")." >".$recordS->fields[$fieldname];
							$recordS->MoveNext();
						}
				}
				...
				else
				{
					...
					while (list($globalValue, $global_list_item) = each ($global_list))
					{
						...
						while (list($indexValue, $list_item) = each ($index_list))
						{
							...
							$s.="<option value='$list_item' ".(($_POST[$field_name]==$list_item||$_GET[$field_name]==$list_item)?"selected":"").">$list_item";
						}
						$scriptstr.= "<script>s_".$field_name."[$ind] = \"$s\";</script>\n";
						$ind++;
					}				
				}
				echo "</select>".$scriptstr."</TD>";
				if ($scriptstr){
			...
			case "number": // handles the input of numbers
				...
				echo "</b></td>";
				echo "<td  class=\"row_main\"><input ".(($field_name=="Cost"||$field_name=="Quantity")?"readonly style='color:gray'":"")." style='width:230px' type=\"text\" name=\"$field_name\" value=\"$default_text\" width=\"100\"></td>";
				break;
	...
	function renderExistingFormElement($field_type, $field_name, $field_value, $field_caption, $default_text, $required, $field_elements,$visible)
	{
		// handles the rendering of already filled in user forms
		global $lang,$conn,$userID,$edit;
		$fname = explode("__",$field_name);
		$dependend_field = trim($fname[1]);
		...
		switch ($field_type)
		{
			...
			case "text": // handles text input boxes
				...
				echo "</b></td>";
				echo "<td  class=\"row_main\"><input style='width:230px' type=\"text\" name=\"$field_name\" value=\"$field_value\" width=\"100\"";
				if ($field_type == 'date')
				{
			...
			case "textarea": // handles textarea input
				...
				echo "</b></td>";
				echo "<td  class=\"row_main\"><textarea style='width:230px'  name=\"$field_name\" cols=40 rows=6>$field_value</textarea></td>";
				break;
			case "select": // handles single item select boxes
				...
				if (strpos(" ".$field_elements,"{")=="1"){
						...
						$s = explode(".",$s);
						$tablename = $s[0];$fieldname=$s[1];
						$ssql = "select ID,$fieldname from $tablename where user_ID = $userID order by SortOrder,$fieldname";
						$recordS = $conn->Execute($ssql);
						while (!$recordS->EOF){
			...
			case "price": // handles price input
				...
				echo "</b></td>";
				echo "<td  class=\"row_main\">$money_sign<input style='width:230px' type=\"text\" name=\"$field_name\" value=\"$field_value\" width=\"100\"> .00 </td>";
				break;
			case "url": // handles url input fields
				...
				echo "</b><BR>($lang[dont_forget_http])</td>";
				echo "<td  class=\"row_main\"><input type=\"text\" name=\"$field_name\" value=\"$field_value\" width=\"100\"></td>";
				break;
			...
			case "number": // deals with numbers
				...
				echo "</b></td>";
				echo "<td  class=\"row_main\"><input ".(($field_name=="Cost"||$field_name=="Quantity")?"readonly style='color:gray'":"")." style='width:".(($field_name=="Cost")?"130":"230")."px' type=\"text\" name=\"$field_name\" value=\"$field_value\" width=\"100\">";
				if ($field_name=="Cost"){
					echo "<input style='width:100px;color=blue' type=button value='שינוי עלות' onclick='s=window.open(\"sfira.php?ids=$edit\",\"sfira\",\"top=\"+(window.screenTop+15)+\",left=\"+(window.screenLeft+15)+\",width=500,height=400\");s.focus();'>";
	...
	{ // handles multiple page listings
		...
		echo "<table border=\"0\" cellspacing=\"0\" cellpadding=\"2\" ><TR><TD nowrap align=left>";
		/*
		if ($num_rows == 1) 
		{
			echo "$lang[there_is_currently] <B>$num_rows</b> $lang[listing].";
		}
		else {
			echo "$lang[there_are_currently] <B>$num_rows</b> $lang[listings] $lang[that_match_search].";
		}
		*/
		...
		if ($total_num_page != 0)
		{
			...
			{
				echo "&nbsp;<a href=\"$PHP_SELF?cur_page=$prevpage&$guidestring\"><strong><<</strong></a></a>&nbsp;";
			} // end if
			if ($total_num_page > 2)
			{
				if ($page_num != 1) 
			...
			{
				echo "&nbsp;<a href=\"$PHP_SELF?cur_page=$nextpage&$guidestring\"><strong>>></strong></a></a>&nbsp;";
			} // end if
		} // end if
	...
	function log_error($sql)
	{
		...
		mail("$config[admin_email]", "SQL Error", $message,"From: $config[admin_email]");
		die("$lang[alert_site_admin]<br>$message");
	} // end function log_action
