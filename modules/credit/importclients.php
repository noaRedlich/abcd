...
פורמט קובץ EXCEL:
-------------------
<b style=color:red>A:</b> מספר לקוח
...
if($excel_file == '') die("<center><b style=color:red>{$lang['select_file']}");


$fh = @fopen ($excel_file,'rb');
if( !$fh ) die("<center><b style=color:red>{$lang['select_file']}");
if( filesize($excel_file)==0 ) die("<center><b style=color:red>{$lang['select_file']}");
...
for ($worksheet=0;$worksheet<count($exc->worksheet['name']);$worksheet++){
	...
	print "<span style='color:blue' >".$lang['to_sheet'].': '.$wname."</span>\n";flush();
		...
		foreach( $data as $i => $row )
		{
			...
			if ($clientnum && $name)
            {
				...
				if ($rs->EOF)
                {
					...
					echo $lang['new_client'].": ". $businessnum." ".$name." ".$warn."\n";flush();
				}
				else
                {
					...
					$prl = $conn->Execute($sql); if ($prl === false){log_error($sql);}
 					...
 					echo $lang['client_edited'].": ". $businessnum." ".$name." ".$warn."\n";flush();
				}
				
			}
			else
            {
				echo "<span style='color:red' dir=ltr>{$lang['err_line_businessnum']}</span>\n";
			}
		}
	}
}
