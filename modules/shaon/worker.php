...
/*sk 10/042016 delete row in nochechut report*/
/*if ($deleterow) {
...
if (isset($_REQUEST['delete_row']) && $_REQUEST['delete_row']) {
    $sql = "delete from attendance where id=" . $_REQUEST['delete_row'] . " and user_id = $userID";
    $upd = $conn->Execute($sql);
    if ($upd === false) {
        log_error($sql);
    }
...
if ($_POST["UPDATE"]) {
    ...
    foreach ($_POST["entertime"] as $time) {
        ...
        $sql = "update attendance set  entermanual = 1, entertime=(case when '" . $atime[0] . "' = '' then null else '" . $atime[0] . "' end) ,entertime_updated=now() where (entertime is null or entertime<>'" . $atime[0] . "') and id=" . $atime[1];
        //echo $sql;
        $upd = $conn->Execute($sql);
        if ($upd === false) {
            log_error($sql);
    ...
    foreach ($_POST["exittime"] as $time) {
        ...
        $sql = "update attendance set exitmanual =1, exittime='" . $atime[0] . "',exittime_updated=now() where (exittime is null or exittime<>'" . $atime[0] . "') and id=" . $atime[1];
        $upd = $conn->Execute($sql);
        if ($upd === false) {
            log_error($sql);
        }
	...
	/*sk 11/02/16*/
	$i = 0;
	foreach ($_POST["exit_day"] as $time) {
		$atime = explode("#",$time);
		  list($day, $month, $year) = explode("/", $atime[0]);
       $ymd = "$year-$month-$day";
		 $sql = "update attendance set exit_day='" . $ymd . "',exittime_updated=now() where (exit_day is null or exit_day<>'" . $atime[0] . "') and id=" . $atime[1];
        $upd = $conn->Execute($sql);
        if ($upd === false) {
            log_error($sql);
        }
...
?>
    ...
    <script>
        ...
        <!--sk 10/04/2016 delete row in nochechut report-->
		function delete_row( id_element){
        	var loc=window.location.href+"&delete_row="+id_element;
...
if ($saction == "sendreport") {
    ...
    if (!$sort) {
        ...
        $sql = "
			...
			entertime_updated,
			exittime_updated,
			DATEDIFF(a.exit_day,a.day) AS date_diff,
			DAYOFWEEK(a.day) as weekday,
			DAYOFWEEK(a.exit_day) as weekday_exit,
				(case when
 ...
 $ii=0;
        while (!$days->EOF) {
				$ii++; //count the length of the array
            $report[] = array(
                "id"                => $days->fields["id"],
                "day"               => $days->fields["day"],
                ...
                "exitmanual"            => $days->fields["exitmanual"],
                "entermanual"            => $days->fields["entermanual"],
                "exit_day"          => $days->fields["exit_day"],
                "date_diff"         => $days->fields["date_diff"],
                "weekday_exit"       => $days->fields["weekday_exit"],
 ...
 //הצגת דוח לפי רצף של שעות ולא לפי ימים
if($show_days_report==0){//sk 01/02/16

	for($j=0;$j<$ii;$j++,$xx++){
		$flag_in=0;
		$exittime=$report[$j]["exittime"];		
		$entertime=$report[$j]["entertime"];
		$day_enter=$report[$j]["day"];
		$day_exit= $report[$j]["exit_day"];
		$date_diff=$report[$j]["date_diff"];
		 $weekday = $report[$j]["weekday"];
		...
		if($date_diff>0){
			...
			/*echo '<hr>';
				echo  'total :  '.$total;
				echo '<hr>';*/
				//$total= ($diff_day-1)*24*60;
				$num_hours=$report[$j]["entertime"];
				echo "enter  ".$num_hours;
		...
		$new_report[$xx] = array(
                ...
                "exittime"          => $exittime,
                "total"             => $final_total,
                "nosafot"           => $report[$j]["nosafot"],
                "entertime_updated" => $report[$j]["entertime_updated"],
                "dayexit"			=> $report[$j]["exit_day"],
                "weekday_exit"      => $report[$j]["weekday_exit"],
                "exittime_updated"  => $report[$j]["exittime_updated"],
                "valid"             => $report[$j]["valid"],
                "salary"            => $report[$j]["salary"],
                "base_salary"       => $report[$j]["base_salary"],
                "nosafot_salary"    => $report[$j]["nosafot_salary"],
                "hour_salary"       => $report[$j]["hour_salary"],
                "holiday_name"      => $report[$j]["holiday_name"],
                "erev_hag"          => $report[$j]["erev_hag"],
                "manual"            => $report[$j]["manual"],
                "exitmanual"            => $report[$j]["exitmanual"],
                "entermanual"            => $report[$j]["entermanual"],
                ""
            );
...
&nbsp;&nbsp;{$lang['identification']}: <b>" . $worker->fields["TeudatZehut"] . "</b>";
    ...
    if (trim($worker->fields["EndDate"]) && $worker->fields["EndDate"] != "0000-00-00") {
       ...
       $show_seq_hour=1;
   if($_SERVER[“REMOTE_ADDR”]==	'87.69.169.217' ){
   	   $show_seq_hour=1;
   }	
	...
	echo "<th class='td_date_out'><a href='$url&sort=weekday'>תאריך יציאה</a></th>
	      <th class='td_day_out'><a href='$url&sort=entertime'>יום יציאה</a></th>";	
	}
echo "<th class='td_output'><a href='$url&sort=exittime'>{$lang['output']}</a></th>
    ...
    foreach ($report as $row) {
		...
		$exit_day= substr($row["dayexit"],8,2) . "/" . substr($row["dayexit"],5,2) . "/" . substr($row["dayexit"],0,4);
		/*sk*/
       // $day_exit= substr($row["day"],8,2) . "/" . substr($row["day"],5,2) . "/" . substr($row["day"],0,4);
        echo "
		...
		<td class='td_login' ". (($row["entermanual"]) ? "style='background-color:#efefef'" : "") ." " . (($row["entertime_updated"]) ? "style=color:blue" : "") . " onclick='edit(event," . $row["id"] . ",\"$entertime\",\"entertime\")'>" . ($entertime) . "</td>";
		if($show_days_report==0){
			echo "<td class='td_date_out' onclick='edit(event," . $row["id"] . ",\"$exit_day\",\"exit_day\")'>".$exit_day."</td>
	      		<td class='td_day_out'>".($days[$row["weekday_exit"]])."</td>";	
	     }
		
	echo "<td class='td_output' " . (($row["exitmanual"]) ? "style='background-color:#efefef'" : "") ." " . (($row["exittime_updated"]) ? "style=color:blue" : "") . " onclick='edit(event," . $row["id"] . ",\"$exittime\",\"exittime\")'>" . ($exittime) . "</td>
       ...
       // echo " <img src=" . $imgPath . "delete.png  align=absmiddle onclick='if (confirm(\"למחוק שורה?\")){document.F.deleterow.value=" . $row["id"] . ";document.F.submit();}' style='cursor:hand' alt='מחק שורה'>";
        echo " <img src=" . $imgPath . "delete.png  align=absmiddle onclick='if (confirm(\"למחוק שורה?\")){delete_row(".$row['id'].")}' style='cursor:hand' alt='מחק שורה'>";
        echo "</td></tr>";
	...
	if($show_days_report==0){
	echo "<td class='td_date_out'></td>
	      <td class='td_day_out'></td>";	
	}
	echo "<td class='td_output'></td>
